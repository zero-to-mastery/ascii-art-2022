...
def get_ascii_art(filepath) -> str:
    try:
        ...
        ascii_ = convert_image_to_ascii(
            img, range_width=range_width, ascii_chars=ASCII_CHARS
        )
        return ascii_
...
def home():
    return {
        "Success": "You can get the ASCII at the end point '/get-ascii' by uploading an image file as 'file' field in a JSON format. You can get raw ASCII art (or as file) at the end point '/get-ascii-file'"
    }
...
async def get_ascii(file: UploadFile = File(...)):
    ...
    if filepath == CodingReturns["unsupported_file_format"]:
        raise HTTPException(
            ...
            detail="The image file type is not matched! Only png, jpg & jpeg are allowed.",
        )
    ...
    if ascii_ == CodingReturns["cant_convert_to_ascii"]:
        raise HTTPException(
            ...
            detail="Can't convert the image to ascii art. Try again.",
        )

    try:
        os.remove(filepath)
        return {
            "success": "Successfully created the ASCII art of the image.",
            "ascii_art": ascii_,
        }
    except Exception as e:
...
def get_ascii_url(image_url: str):
    ...
    if not is_valid:
        raise HTTPException(
            ...
            detail="The url is invalid. Try again using a valid image url.",
        )
    ...
    if filepath == CodingReturns["cant_save_the_image"]:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to save the image! Try again.",
        )
    elif filepath == CodingReturns["url_request_error"]:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Did not get valid response from the image url. Try again",
        )

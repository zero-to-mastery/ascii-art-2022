...
def single_ascii_replacement(image_ascii,singleAsciiChar):
    #Creating a list for getting the count of ASCII character
    asc_count = []
    for asc_char in ["%","?","+","°","@","O","o","#","."":",",","*"," "]:
        asc_count.append((asc_char , image_ascii.count(asc_char)))
    asc_count.sort(key = lambda x:x[1] ,reverse=True)
    chr=asc_count[0][0]
    #Replacing highest ASCII with blank
    new_image_re = re.sub(r"[chr]", " ", image_ascii)
    #Replacing all  ASCII with input ASCII singlecharacter

...
if len(sys.argv)==3: # The user Specefied which CHAR_SET to use or included his/her own
    CHAR_SET = sys.argv[2] # Either an integer value specifying which previously made set to use, or a list value with the characters to use
    ...
    if CHAR_SET[0] == "[" and CHAR_SET[-1] == "]": # is a list
        ...
        try:
            ...
            raise Exception("Please insert a correct value, either an int value to select which CHAR_SET to use, or a list value of characters of your own!")
...
if isinstance(CHAR_SET, list):
    ...
    range_width = ceil((255+1)/len(ASCII_CHARS)) # as the range width is based on the number of ASCII_CHARS we have
...
def convert_image_to_ascii(image, new_width=100, range_width=range_width):
